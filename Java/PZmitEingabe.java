	...
	public static void main(String[] args) {
		...
		*/
		...
		for (int i = 0; i < idCard.length(); i++){
			idCardNumber[i] = idCard.charAt(i) - '0'; 
			if (idCardNumber[i] < 0 || idCardNumber[i] > 9){
				do {
				idCard = JOptionPane.showInputDialog(
					"Die eingebene ID-Card-Nummern hat falsche character, soll nur Zahlen haben. \n\nBitte neu eingeben:" );
				idCardNumber[i] = idCard.charAt(i) - 48;	
				} while (idCardNumber[i] < 0 || idCardNumber[i] > 9) 
			}
		}		
